...
- [x] Добавить barrier-задачу для каждой partition-очереди
- [x] Дождаться обработки всех initial-задач
- [x] Печатать первоначальный индекс только после barrier
- [x] Проверить создание, изменение и удаление файла во время initial scan
...
Гарантия завершения initial scan:
...
- [x] Сделать замену `ScheduledFuture` в Map атомарной
- [x] Не позволять старой задаче удалить ссылку на новую
- [x] Удалять завершённую задачу из Map в `finally`
- [x] Добавить тест серии быстрых `MODIFY` одного пути
- [x] Добавить тест конкурентных вызовов debounce
...
- [x] Создать общий тип задач Worker
- [x] Сделать `FileTask` файловой задачей общего типа
- [x] Добавить отдельную `StopTask`
- [x] Добавить отдельную `BarrierTask`
- [x] Перевести очереди и Worker на общий тип
- [x] Убрать magic string `"STOP"`
- [x] Запретить `null` в компонентах `FileTask`
- [x] Проверить, что служебную задачу нельзя спутать с настоящим файлом
...
- [x] `StopTask` и `BarrierTask`
- [x] Fail-fast проверки `FileTask` и `BarrierTask`
- [x] Создание, изменение и удаление файла во время initial scan
- [x] Серия быстрых `MODIFY` одного пути создаёт одну итоговую задачу
- [x] Конкурентные вызовы debounce создают одну итоговую задачу
...
1. [x] Зафиксировать этап 10 отдельным коммитом
2. [x] Создать общий тип `WorkerTask`
3. [x] Подключить к нему `FileTask`
4. [x] Добавить `StopTask`
5. [x] Добавить `BarrierTask`
6. [x] Перевести очереди и Worker на новый тип
7. [x] Убрать `"STOP"` и `null`
8. [x] Добавить barrier initial scan в `Main`
